...
// adiciona mascara ao telefone
function formataMascaraCel(tel) {
	if (mascaraInteiro(tel) == false) {
		event.returnValue = false;
	}
...
function atualizaTermosECondicoes(comboId, calloutId) {
	if (document.getElementById(comboId).checked) {
		$("#"+calloutId).removeClass("well-warning");
		$("#"+calloutId).addClass("well-success");
	} else {
		$("#"+calloutId).removeClass("well-success");
		$("#"+calloutId).addClass("well-warning");
	}
}
